...
class Solution {
    ...
    int solve(vector<int>&prices,int index,int transactionType){
        ...
        // skip this days - (no buy/sell)
        int ans1 = solve(prices,index+1,transactionType);

        // do all possible transactions  - buy or sell for 1 tranxation (1 buy or sell)
        int ans2 = 0;

        if( transactionType == BUY){
            ans2 = -prices[index]+solve(prices,index+1,SELL);
        }else{
            ans2 = +prices[index]+solve(prices,index+1,BUY);//break- as only one transaction (buy+sell) allowed
        }
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        return solve(prices,0,0);//{index,transactionType}
    }
};